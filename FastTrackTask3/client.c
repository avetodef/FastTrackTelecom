...
int main() {
    ...
    Message coded_setup = {buff, bytes_read};
    RRCConnectionSetup_t* connection_setup = DecodeSetup(coded_setup);
    printf("Connection setup ID: %ld\n", connection_setup->rrc_TransactionIdentifier);

    printf("Sending setup complete message");
    Message setup_complete = CodeComplete();
